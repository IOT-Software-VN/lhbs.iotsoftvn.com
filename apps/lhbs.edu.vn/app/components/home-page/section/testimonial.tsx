...
export default function TestimonialsSection() {
  ...
  const testimonials: Testimonial[] = [
    {
      quote:
        'LHBS has been an incredible experience for our family. Our daughter has grown confident and expressive in both English and Vietnamese. We truly appreciate how the school nurtures her character as much as her academic progress.',
    ...
    {
      quote:
        'We chose LHBS for its bilingual program, and it has exceeded all expectations. Our son now communicates naturally in two languages, and we love how the teachers encourage creativity and global awareness every day.',
    ...
    {
      quote:
        'The teachers at LHBS truly care about every child. They create a supportive and inspiring environment that helps our kids discover their strengths and become confident learners prepared for the future.',
  ...
  return (
    <motion.section
      ref={ref}
      className='py-24 w-full px-4 md:px-10'
      initial={{ opacity: 0 }}
...
function T_Card({ quote, avatar, avatarAlt, name, role }: T_CardProps) {
  return (
    <div className='bg-white p-5 md:p-6 shadow-sm hover:shadow-md transition-shadow duration-300 h-full flex flex-col'>
      ...
      {/* Quote Text */}
      <blockquote className=' text-base text-[#212121] mb-4 leading-relaxed grow'>"{quote}"</blockquote>

      {/* Author Block */}
      <div className='flex items-center gap-3 pt-4 border-t border-[#1a5336]/10'>
        {/* Avatar */}
        <div className='w-12 h-12 overflow-hidden shrink-0'>
          <ImageWithFallback src={avatar} alt={avatarAlt} className='w-full h-full object-cover' />
        </div>

        {/* Name & Role */}
        <div className='flex-1 min-w-0'>
          <p className=' text-sm text-[#1a5336] font-semibold truncate'>{name}</p>
          <p className=' text-xs text-[#212121]/70 line-clamp-2'>{role}</p>
        </div>
      </div>
    </div>
  )
