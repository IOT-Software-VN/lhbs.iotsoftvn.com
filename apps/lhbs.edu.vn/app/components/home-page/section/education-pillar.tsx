...
export default function EducationPillarsSectionComponent({
  ...
  const col1: (StatCardData | PhotoCardData)[] = [
    ...
    } as StatCardData,
    {
      ...
      subtitle:
        'LHBS students achieved remarkable success in the prestigious NASA Space Apps Challenge with their innovative ATMOS GUARD project, showcasing their scientific excellence and global competitiveness.'
    } as PhotoCardData
  ]
  ...
  const col2: (StatCardData | PhotoCardData)[] = [
    {
      ...
      subtitle:
        'Students engage in innovative computer-based learning activities that promote collaboration, critical thinking, and 21st-century skills development in our modern classrooms.'
    } as PhotoCardData,
    {
  ...
  const col3: (StatCardData | PhotoCardData)[] = [
    ...
    } as StatCardData,
    {
      ...
      subtitle:
        'Our students actively participate in hands-on STEM robotics projects, developing engineering skills, programming abilities, and innovative problem-solving techniques for the digital age.'
    } as PhotoCardData
  ]
  ...
  const col4: (StatCardData | PhotoCardData)[] = [
    ...
    } as StatCardData,
    {
      ...
      subtitle:
        'LHBS graduation ceremony celebrates outstanding student achievements, marking the successful completion of their bilingual education journey and readiness for global opportunities.'
    } as PhotoCardData,
    {
  ...
  return (
    <section ref={ref} className='relative py-24 px-4 md:px-10 w-full overflow-hidden'>
        ...
        {columns.map((column, colIndex) => (
          ...
          >
            {column.map((card, cardIndex) => (
              <div key={cardIndex}>
                {isStatCard(card) ? <StatCard {...card} /> : <PhotoCard {...card} setShowModal={setShowModal} />}
              </div>
            ))}
          </motion.div>
        ))}
...
function StatCard({ icon, value, body, isExtended, minHeight }: StatCardData) {
  return (
    ...
    >
      {/* Icon - size lớn hơn */}
      <div className='text-[#fffae9] text-5xl md:text-6xl lg:text-7xl mb-4 scale-125 md:scale-150'>{icon}</div>

      {/* Value - size lớn hơn */}
      <h3 className='font-serif text-5xl md:text-6xl lg:text-5xl text-white mb-4'>{value}</h3>

      {/* Body - size lớn hơn */}
      <p className='text-base md:text-lg text-white/90 leading-relaxed max-w-sm'>{body}</p>
    </div>
  )
}
...
function PhotoCard({
  ...
  return (
    <div className='relative overflow-hidden cursor-pointer hover:opacity-90 transition-opacity' onClick={handleClick}>
      <img src={image} alt={alt} className='w-full h-auto object-cover' style={{ aspectRatio: `1 / ${heightRatio}` }} />
      {title && (
...
export function EducationPillarModal({ image, alt, title, subtitle, onClose }: EducationPillarModalProps) {
  // Handle Escape key
  useEffect(() => {
    ...
    >
      ...
      >
        ...
        <div className='p-8 md:p-12'>
          <h2 className='text-3xl md:text-4xl font-bold text-[#1a5336] mb-4'>{title}</h2>

          <p className='text-lg md:text-xl text-[#212121] leading-relaxed'>{subtitle}</p>
        </div>
      </motion.div>
    </motion.div>
  )
