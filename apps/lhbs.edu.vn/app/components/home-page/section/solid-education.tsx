...
export function SolidEducationSection({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  return (
    <motion.section
      ...
      ref={ref}
      className='relative w-full bg-white overflow-hidden flex justify-center'
      initial={{ opacity: 0 }}
    ...
    >
      ...
      <div className='w-full px-4 md:px-10 grid md:grid-cols-2 gap-16 items-center relative z-10'>
        ...
        >
          <h2 className="text-[#1a5336] mb-6 font-['SVN-Gotham']" style={{ fontSize: '48px', lineHeight: '1.24' }}>
            {/* Phần tiêu đề chính có sự khác biệt */}
            <span
              className='block mb-3 text-[#FABA1E] tracking-widest uppercase font-medium'
              style={{
                fontSize: '22px',
                letterSpacing: '2px'
              }}
            ...
            <AnimatedHighlight delay={1.5} duration={1.5} backgroundColor='#95F77E' className='text-[#1a5336]'>
              A Solid Education
            </AnimatedHighlight>

            <span className='block mt-2 text-[#1a5336]/90'>for an Innovative Future</span>
          </h2>

          {/* Body copy */}
          <div className='text-base md:text-lg text-[#212121] mb-8 leading-relaxed max-w-[70ch]'>
            <p className='mb-6'>
              Lac Hong Bilingual School cultivates lifelong learning, nurtures character, and builds global readiness
              through a holistic bilingual education that harmonizes Vietnamese cultural values with international
              excellence. The school empowers students to develop intellectually, emotionally, and globally, enabling
              them to live with compassion, confidence, and a strong sense of contribution to society.
            </p>
          </div>
...
function EducationLevelItem({ label, link, onNavigate, delay, isInView }: EducationLevelItemProps) {
  return (
    ...
    >
      <span className=' text-lg md:text-xl text-[#1a5336]'>{label}</span>
      <motion.svg
