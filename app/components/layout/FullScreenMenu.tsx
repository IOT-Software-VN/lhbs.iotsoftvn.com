...
import { siteNavigation, moreLinks, schoolLinks } from '@/types/navigation'
                        ...
                        {activeParent.children.map((child) => (
                          child.isExternal && child.url ? (
                            ...
                            <button
                              ...
                              className={`block w-full text-left text-[15px] transition-colors group ${currentPath === child.path ? 'text-brand-gold' : 'text-white hover:text-brand-gold'
                                }`}
                            >
                              <div className='flex items-center justify-between'>
                                <span className='truncate'>{child.label}</span>
                              ...
                              {parent.children.map((child) => (
                                child.isExternal && child.url ? (
                                  ...
                                  <button
                                    key={child.id}
                                    onClick={() => handleNavClick(child.path!)}
                                    className={`block w-full text-left py-2  text-[15px] ${currentPath === child.path ? 'text-brand-gold' : 'text-white/80 hover:text-white'
                                      }`}
                                  >
                                    {child.label}
                                  </button>
